...
result_id_without_match <- result_project_read %>%
  filter(verified_id) %>%
  anti_join(survey_project_read,
    by = c("record_id" = "research_encounter_id")
  ) %>%
...
if (nrow(swab_result) > 0) {
  ...
  body <-
    paste0(
      "The attached file(s) includes a log of all results that were uploaded",
