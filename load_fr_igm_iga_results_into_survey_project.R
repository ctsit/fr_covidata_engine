...
survey_project_read <- redcap_read_oneshot(redcap_uri = Sys.getenv("URI"),
  ...
  select(record_id, redcap_event_name, research_encounter_id, 
         covid_19_swab_result, igg_antibodies, igm_antibodies, iga_antibodies)
...
result_project_read <- redcap_read_oneshot(redcap_uri = Sys.getenv("URI"),
  ...
  mutate(iga_antibodies = as.character(iga_antibodies)) 
...
result_id_with_bad_checksum <- result_project_read %>% 
  ...
  select(record_id, igg_antibodies, igm_antibodies, iga_antibodies, verified_id,
         reason_not_imported)
...
lab_result <- result_project_read %>% 
  ...
  select(research_encounter_id = record_id, igg_antibodies, igm_antibodies, iga_antibodies, verified_id) %>%
  filter(!is.na(igg_antibodies)) %>%  
  ...
  mutate(igm_antibodies = case_when(
    str_detect(str_to_lower(igm_antibodies), "pos") ~ "1",
    str_detect(str_to_lower(igm_antibodies), "neg") ~ "0",
    str_detect(str_to_lower(igm_antibodies), "indeterminate") ~ "2",
    str_detect(str_to_lower(igm_antibodies), "inadequate") ~ "99",
    TRUE ~ igm_antibodies
  )) %>%
  mutate(iga_antibodies = case_when(
    str_detect(str_to_lower(iga_antibodies), "pos") ~ "1",
    str_detect(str_to_lower(iga_antibodies), "neg") ~ "0",
    str_detect(str_to_lower(iga_antibodies), "indeterminate") ~ "2",
    str_detect(str_to_lower(iga_antibodies), "inadequate") ~ "99",
    TRUE ~ iga_antibodies
  )) %>%
  select(record_id, redcap_event_name, igg_antibodies, igm_antibodies, iga_antibodies, verified_id) %>%
  arrange(record_id) 
...
if (nrow(lab_result) > 0){
  ...
  lab_result_to_import <- lab_result %>% 
    filter(igm_antibodies %in% c("1","0","2","99") & verified_id) %>% 
    select(-verified_id)
